...
**Mijn eigen misser (gecorrigeerd):** ik had namen + e-mailadressen van contactpersonen in de OPENBARE repo (prospects.md/brein.md) gezet. Openbaar publiceren van persoonsgegevens is een aparte, moeilijk te rechtvaardigen verwerking (doelbinding). → **Actie:** alle persoonsnamen en specifieke adressen uit de openbare bestanden gehaald; verplaatst naar een git-ignored lokaal bestand (outreach/contacten-prive.md). **Open punt voor facilitator:** de git-historie bevat de oude gegevens nog; volledig wissen vereist een history-rewrite + force-push (destructieve actie) — beslissing aan facilitator. Risico beperkt: het betrof zakelijke contactgegevens die ook op de eigen bedrijfssites staan.
